...
--
-- Host: 127.0.0.1
-- Generation Time: 19-Set-2019 às 19:38
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `addAluno_matriculado` (IN `_apelido` VARCHAR(30), IN `_nomes` VARCHAR(50), IN `_pais_nas` VARCHAR(50), IN `_provincia_nas` VARCHAR(50), IN `_distrito_nas` VARCHAR(50), IN `_data_nascimento` DATE, IN `_bi` VARCHAR(13), IN `_local_emissao` VARCHAR(50), IN `_data_em` DATE, IN `_sexo` ENUM('M','F'), IN `_estado_civil` ENUM('S','C'), IN `_provincia_res` VARCHAR(50), IN `_distrito_res` VARCHAR(50), IN `_bairro` VARCHAR(50), IN `_av_ou_rua` VARCHAR(50), IN `_quarteirao` VARCHAR(50), IN `_nr_casa` INT, IN `_numero_telf` INT, IN `_email` VARCHAR(50), IN `_nome_pai` VARCHAR(50), IN `_telefone_pai` INT, IN `_local_trabalho_pai` VARCHAR(50), IN `_profissao_pai` VARCHAR(50), IN `_nome_mae` VARCHAR(50), IN `_telefone_mae` INT, IN `_local_trabalho_mae` VARCHAR(50), IN `_profissao_mae` VARCHAR(50), IN `_nome_enc` VARCHAR(50), IN `_numero_tf_enc` INT, IN `_provincia_enc` VARCHAR(50), IN `_distrito_enc` VARCHAR(50), IN `_bairro_enc` VARCHAR(50), IN `_av_ou_rua_enc` VARCHAR(50), IN `_local_trab_enc` VARCHAR(50), IN `_profissao_enc` VARCHAR(50))  Begin
	...
	if (idp_p_t is not null) then
		...
		Begin
			...
			Call addPessoa(7,_nomes,_apelido,_data_nascimento,_sexo, 
			...
			if(idp_p is not null) then
				Begin
					...
					Insert into aluno (id_aluno,id_pessoa,id_natural) values (3,idp_p,id_nat);
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `addCandidato` (IN `_id_cand` INT, IN `_nomes` VARCHAR(50), IN `_apelido` VARCHAR(50), IN `_data_nascimento` DATE, IN `_sexo` ENUM('M','F'), IN `_classe_matricular` ENUM('8','9','10','11','12'), IN `_regime` ENUM('Diurno','Nocturno'), IN `_escola_Origem` VARCHAR(50), IN `_distrito_origem` VARCHAR(50))  Begin 
	declare id_esco int;
	declare id_matr int;
...
(3, '1212112131e', '2019-09-04', 'Cidade de Maputo');
...
(5, 1, 'Claudio Bucene', 'Bucene', '2019-09-17', 'M', '8', 'Diurno', 2019, 'Claudio BuceneBucene', 'xÆØ±¡X%H›-è¹XˆÑû');
...
(8, 3, 'asda;ldmadada', 2147483647, 'Cidade de Maputo', 'asdadadadad');
...
(8, 3, 'Daniel Bucene', 1212312312, 'Cidade de Maputo', 'sadjksadas', 'sajkdnakdnada', 1232111231, 'Cidade de Maputo', 'asdadadasd');
...
(7, 'Claudio Bucene', 'Bucene', '2019-09-17', 'M', 'C', 2147483647, 'dddd@gmail.com', '2019-09-19');
...
(3, 1, 'Av. de Mocambique', '31313', 2131);
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `candidatos`  AS  select `candidato_aluno`.`id_candidato` AS `id_candidato`,concat(`candidato_aluno`.`nome`,' ',`candidato_aluno`.`apelido`) AS `nome_completo`,`candidato_aluno`.`regime` AS `regime`,`candidato_aluno`.`classe_matricular` AS `classe_matricular`,`candidato_aluno`.`ano` AS `ano` from `candidato_aluno` ;
...
--
ALTER TABLE `encarregado`
  MODIFY `id_encarregado` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=9;
...
--
ALTER TABLE `filiacao`
  MODIFY `id_filiacao` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=9;
